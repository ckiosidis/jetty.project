...
package org.eclipse.betty.http;
...
import org.eclipse.betty.http.HttpTokens.EndOfContent;
 ...
 * <p>
 * If the system property "HttpGenerator.STRICT" is set to true,
...
public class HttpGenerator
{
    ...
    public final static boolean __STRICT=Boolean.getBoolean("HttpGenerator.STRICT");
    ...
    public Result generateRequest(MetaData.Request info, ByteBuffer header, ByteBuffer chunk, ByteBuffer content, boolean last) throws IOException
    {
        switch(_state)
        {
            case START:
            {
                ...
                {
                    throw new BadMessageException(HttpStatus.INTERNAL_SERVER_ERROR_500,e.getMessage(),e);
                }
                finally
                {
    ...
    public Result generateResponse(MetaData.Response info, boolean head, ByteBuffer header, ByteBuffer chunk, ByteBuffer content, boolean last) throws IOException
    {
        switch(_state)
        {
            case START:
            {
                ...
                {
                    throw new BadMessageException(HttpStatus.INTERNAL_SERVER_ERROR_500,"Response header too large",e);
                }
                catch(Exception e)
                {
                    throw new BadMessageException(HttpStatus.INTERNAL_SERVER_ERROR_500,e.getMessage(),e);
                    ...
                    switch (h)
                    {
                        case CONTENT_LENGTH:  
                            if (content_length<0)
                                ...
                                throw new BadMessageException(HttpStatus.INTERNAL_SERVER_ERROR_500,String.format("Incorrect Content-Length %d!=%d",content_length,field.getLongValue()));
